...
class DDPG(nn.Module):
    def __init__(self, writer, device, state_dim, action_dim, args, noise):
        ...
        self.q = Critic(self.args.layer_num, state_dim + action_dim, 1, self.args.hidden_dim,
                        self.args.activation_function, None)

        self.target_q = Critic(self.args.layer_num, state_dim + action_dim, 1, self.args.hidden_dim,
                               self.args.activation_function, None)
    ...
    def train_net(self, batch_size, n_epi):
        ...
        targets = rewards + self.args.gamma * (1 - dones) * self.target_q(next_states,
